    ...
    <form class="flex flex-col space-y-2 md:space-y-4" wire:submit.prevent="submit">
        ...
        <div class="form-group">
            ...
            <select
                ...
                id="plot"
                wire:model="plot"
                class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray disabled:opacity-25"
            >
                <option value="">Select Plot</option>
                @forelse($plots as $p)
                    <option value="{{$p->id}}">{{$p->no}} : {{$p->name}}</option>
                @empty
